...
function DebtPlanner({ user, db, handleSignOut, theme, toggleTheme }) {
    ...
    useEffect(() => {
        ...
        const unsubFinancials = onSnapshot(doc(db, financialDocPath), (doc) => {
            if (doc.exists()) setIncome(doc.data().income || 0);
        });
    ...
    const { totalExpenses, totalMinPayment, availableForDebt, leftToSpend } = useMemo(() => {
        ...
        return { totalExpenses: exp, totalMinPayment: minPay, availableForDebt: Math.max(0, available), leftToSpend: Math.max(0, discretionary) };
    }, [income, expenses, debts, extraPayment]);
...
function ReportsPage({ db, userId, achievements, debts, expenses }) {
                    ...
                    {expenses.map(exp => (
                        <div key={exp.id} className="flex justify-between items-center p-2 bg-slate-50 dark:bg-slate-700/50 rounded-md">
                            <div>
                                ...
                                <span className="text-xs ml-2 text-slate-500 dark:text-slate-400 bg-slate-200 dark:bg-slate-600 px-2 py-0.5 rounded-full">{exp.category}</span>
                            </div>
...
function SpendingAnalysisCard({ expenses }) {
    const chartData = useMemo(() => {
        ...
        const COLORS = ['#FF8042', '#00C49F', '#0088FE', '#FFBB28', '#8884d8', '#82ca9d'];
