...
import { Trash2, TrendingUp, DollarSign, PlusCircle, Pencil, BarChart2, PieChart, Zap, ClipboardPlus, History, Target, ListTree, Lightbulb, Sun, Moon, LogOut } from 'lucide-react';
...
function DebtPlanner({ user, db, handleSignOut, theme, toggleTheme }) {
    ...
    const updateIncome = async (newIncome) => {
        if (!db || !user) return;
        const financialDocPath = `artifacts/${appId}/users/${user.uid}/financials/profile`;
        await setDoc(doc(db, financialDocPath), { income: newIncome }, { merge: true });
    };
    ...
    const { totalExpenses, totalMinPayment, totalBalance, availableForDebt } = useMemo(() => {
        ...
        const bal = debts.reduce((sum, debt) => sum + parseFloat(debt.balance || 0), 0);
        const available = income - exp - minPay;
        return { totalExpenses: exp, totalMinPayment: minPay, totalBalance: bal, availableForDebt: Math.max(0, available) };
        ...
        <div className="container mx-auto p-4 sm:p-6 lg:p-8">
            ...
            <main className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-6">
                <div className="lg:col-span-2 space-y-8">
                    <BudgetCard income={income} expenses={expenses} onUpdateIncome={updateIncome} onAddExpense={addExpense} onDeleteExpense={deleteExpense} />
...
function BudgetCard({ income, expenses, onUpdateIncome, onAddExpense, onDeleteExpense }) {
    ...
    const handleIncomeBlur = () => {
        onUpdateIncome(parseFloat(currentIncome) || 0);
    };

    const handleAddExpense = (e) => {
        e.preventDefault();
        if (!expenseName || !expenseAmount) return;
        onAddExpense({ name: expenseName, amount: parseFloat(expenseAmount) });
        setExpenseName('');
        setExpenseAmount('');
    };
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
            ...
            {/* Income Section */}
            <div className="mb-6">
                <label htmlFor="monthly-income" className="block text-sm font-medium text-slate-700 dark:text-slate-300">Total Monthly Income</label>
                <div className="relative mt-1">
                    <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" size={16} />
                    <input id="monthly-income" type="number" value={currentIncome} onChange={e => setCurrentIncome(e.target.value)} onBlur={handleIncomeBlur} className="w-full p-2 pl-9 border rounded-md bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600 focus:ring-2 focus:ring-blue-500" step="100" min="0"/>
                </div>
            </div>
            ...
            <div>
                ...
                <form onSubmit={handleAddExpense} className="flex gap-2">
                    <input type="text" value={expenseName} onChange={e => setExpenseName(e.target.value)} placeholder="Expense Name" className="flex-grow p-2 border rounded-md bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600" required />
                    <input type="number" value={expenseAmount} onChange={e => setExpenseAmount(e.target.value)} placeholder="Amount" className="w-28 p-2 border rounded-md bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600" required step="0.01" min="0" />
                    <button type="submit" className="bg-blue-500 text-white p-2 rounded-md hover:bg-blue-600"><PlusCircle size={20}/></button>
                </form>
            </div>
...
// --- MODIFIED StrategySelector ---
function StrategySelector({ strategy, setStrategy, extraPayment, setExtraPayment, availableForDebt }) {
    useEffect(() => {
        // When available funds change, ensure extra payment doesn't exceed it.
...
// --- Other components (Header, DebtList, Modals, etc.) are unchanged from the previous version ---
// For brevity, I'm omitting the components that do not need any changes.
// The following components are included as they are called by the main planner.
